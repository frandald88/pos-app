...
router.post("/checkin", verifyToken, identifyTenant, requireTenant, async (req, res) => {
  try {
...
router.post("/checkout", verifyToken, identifyTenant, requireTenant, async (req, res) => {
  try {
    const userId = req.body.userId || req.userId;
    const today = new Date();
    const startOfDay = new Date(today.setHours(0, 0, 0, 0));
...
router.get("/schedule-check", verifyToken, identifyTenant, requireTenant, async (req, res) => {
  try {
...
router.post("/absence", verifyToken, identifyTenant, requireTenant, requireAdmin, async (req, res) => {
  try {
...
router.get("/report", verifyToken, identifyTenant, requireTenant, requireAdmin, async (req, res) => {
  try {
...
router.get("/today", verifyToken, identifyTenant, requireTenant, async (req, res) => {
  try {
...
router.get("/mine", verifyToken, identifyTenant, requireTenant, async (req, res) => {
  try {
