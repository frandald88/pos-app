...
class ClientesController {
  ...
  async create(req, res) {
    try {
      ...
      const cliente = new Cliente({
        ...
        email: email?.trim(),
        tenantId: req.tenantId // Asignar el tenant
      });
  ...
  async update(req, res) {
    try {
      ...
      const clienteExistente = await Cliente.findOne({ _id: id, tenantId: req.tenantId });
      if (!clienteExistente) {
        return errorResponse(res, 'Cliente no encontrado', 404);
      }
  ...
  async delete(req, res) {
    try {
      ...
      // const ventasAsociadas = await Sale.findOne({ cliente: id, tenantId: req.tenantId });
      // if (ventasAsociadas) {
      //   return errorResponse(res, 'No se puede eliminar el cliente porque tiene ventas asociadas', 400);
      // }

      await Cliente.findOneAndDelete({ _id: id, tenantId: req.tenantId });
